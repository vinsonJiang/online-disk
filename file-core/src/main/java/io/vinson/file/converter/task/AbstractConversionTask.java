...
import static io.vinson.file.converter.util.FileUtils.toUrl;
import static io.vinson.file.converter.util.PropertyUtils.toUnoProperties;
...
public abstract class AbstractConversionTask implements OfficeTask {
    ...
    public void execute(OfficeConnection context) throws OfficeException {
    ...
    private XComponent loadDocument(OfficeConnection context, File inputFile) throws OfficeException {
        if (!inputFile.exists()) {
            throw new OfficeException("input document not found");
        }
        XComponentLoader loader = OfficeUtils.cast(XComponentLoader.class, context.getService(SERVICE_DESKTOP));
    ...
    private void storeDocument(XComponent document, File outputFile) throws OfficeException {
        ...
        try {
            OfficeUtils.cast(XStorable.class, document).storeToURL(toUrl(outputFile), toUnoProperties(storeProperties));
        } catch (ErrorCodeIOException errorCodeIOException) {
