...
import io.vinson.file.converter.process.ProcessQuery;
import io.vinson.file.converter.util.FileUtils;
...
import static io.vinson.file.converter.process.ProcessManager.PID_UNKNOWN;
...
public class OfficeProcess {
    ...
    public void start(boolean restart) throws IOException {
        ...
        if (!(existingPid == ProcessManager.PID_NOT_FOUND || existingPid == PID_UNKNOWN)) {
        ...
        command.add("-env:UserInstallation=" + FileUtils.toUrl(instanceProfileDir));
